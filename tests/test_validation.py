...
class _ComparisonResult:
    ...
    def position_agreement(self) -> float:
        return (
            self.position_close / self.position_total * 100.0
    ...
    def font_agreement(self) -> float:
        return (
            self.font_matched / self.font_total * 100.0
            if self.font_total > 0
            else 0.0
        )


def _compare(
    ours: list[CharInfo], pdfminer: list[CharInfo],
) -> _ComparisonResult:
...
class TestPdfminerValidation:
    ...
    def test_position_agreement(self, comparison: _ComparisonResult) -> None:
        ...
        print(
            f"  close={comparison.position_close}, "
            f"far={far}"
        )
    ...
    def test_font_agreement(self, comparison: _ComparisonResult) -> None:
        ...
        print(
            f"  matched={comparison.font_matched}, "
            f"total={comparison.font_total}"
        )
